...
many.push(late);
const seen = [];
const poll = setInterval(async () => { try { seen.push(await page.evaluate(() => window.__hike.progress())); } catch {} }, 20);
await page.setInputFiles('#files', many);
await page.waitForFunction(() => document.getElementById('msg').textContent.includes('added'));
clearInterval(poll);
const shown = seen.filter(p => p.shown && /^Reading photos/.test(p.label));
check(shown.length > 0 && shown.some(p => /\d+ of 120 · \d+%/.test(p.label)) && shown.every(p => !p.busy), `progress bar with counts while reading (${shown.length} samples, e.g. "${shown[Math.floor(shown.length / 2)] && shown[Math.floor(shown.length / 2)].label}")`);
const pAfter = await page.evaluate(() => window.__hike.progress());
check(!pAfter.shown, 'progress bar hidden when done');
...
check(shown.some(p => parseInt(p.width, 10) > 0 && parseInt(p.width, 10) < 100), 'bar fill advances between 0 and 100 %');
